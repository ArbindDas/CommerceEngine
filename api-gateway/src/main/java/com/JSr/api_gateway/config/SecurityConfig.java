...
public class SecurityConfig {
    ...
    public SecurityWebFilterChain securityWebFilterChain(ServerHttpSecurity http) {
        return http
                .csrf(ServerHttpSecurity.CsrfSpec::disable)
                .authorizeExchange(exchanges -> exchanges
                        .pathMatchers("/api/public/**", "/eureka/**", "/actuator/health", "/actuator/info").permitAll()
                        .pathMatchers("/api/inventory/**", "/api/order/**", "/api/product/**", "/api/test/**")
                        .authenticated()
                        .pathMatchers("/api/admin/**").hasRole("ADMIN")
                        .anyExchange().authenticated()
    ...
    public org.springframework.security.oauth2.server.resource.authentication.JwtAuthenticationConverter jwtAuthenticationConverter() {
        ...
        converter.setJwtGrantedAuthoritiesConverter(jwt -> {
            ...
            if (resourceAccess != null) {
                @SuppressWarnings("unchecked")
                Map<String, Object> client = (Map<String, Object>) resourceAccess.get("spring-cloud-client");
                if (client != null) {
                    @SuppressWarnings("unchecked")
                    List<String> clientRoles = (List<String>) client.get("roles");
                    if (clientRoles != null) {
